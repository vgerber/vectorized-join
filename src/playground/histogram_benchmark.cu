...
struct HistogramBenchmarkConfig {
    ...
    HistogramBenchmarkConfig(int runs, int threads_per_block, int elements_per_thread, int elements, int bins, int bin_bytes, int element_bytes, std::string kernel) {
        this->runs = runs;
    ...
    static std::string to_string_header() {
        ...
        string_stream << "threads_per_block,"
                      << "elements_per_thread,"
                      << "elements,"
                      << "bins,"
                      << "bin_bytes,"
    ...
    std::string to_string() {
        std::ostringstream string_stream;
        string_stream << threads_per_block << "," << elements_per_thread << "," << elements << "," << bins << "," << bin_bytes << "," << kernel << "," << element_bytes;
        return string_stream.str();
    }
};
...
struct HistogramBenchmarkResult {
    ...
    static std::string to_string_header() {
        std::ostringstream string_stream;
        string_stream << HistogramBenchmarkConfig::to_string_header() << ","
                      << "gb_p_second,"
                      << "elements_p_second";
        return string_stream.str();
    }
    ...
    std::string to_string() {
        std::ostringstream string_stream;
        string_stream << config.to_string() << "," << gb_p_second << "," << elements_p_second;
        return string_stream.str();
    }
};
                        ...
                        for (auto bin_byte_count : bin_bytes) {
                            for (auto element_byte_count : element_bytes) {
                                for (auto kernel : kernels) {
                                    configs.push_back(HistogramBenchmarkConfig(runs, threads_per_block_count, elements_per_thread_count, element_count, bin_count, bin_byte_count, element_byte_count, kernel));
                                }
                            }
                        }
                    }
...
bool load_histogram_setup(std::string path, std::string profile, HistogramBenchmarkSetup &setup) {
    ...
    if (config_file.contains(profile)) {
        ...
        std::cout << "Read " << field << std::endl;
        if (config_file.at(profile).contains(field)) {
            setup.kernels = toml::find<std::vector<std::string>>(config_file, profile, field);
        } else {
            std::cout << profile << "." << field << " not found" << std::endl;
            return false;
        }
...
template <class B_T, class H_T> __global__ void histogram_kernel(int buffer_size, B_T *buffer, int bins, H_T *histogram) {
...
template <class B_T, class H_T> __global__ void histogram_kernel_shared(int buffer_size, B_T *buffer, int bins, H_T *histogram) {
    extern __shared__ char *shared[];
    H_T *shared_histogram = (H_T *)&shared[0];
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = gridDim.x * blockDim.x;

    for (int element_index = index; element_index < buffer_size; element_index += stride) {
        int key = buffer[element_index] % bins;
        atomicAdd(&shared_histogram[key], (H_T)1);
    }

    __syncthreads();
    for (int bin_index = threadIdx.x; bin_index < bins; bin_index += blockDim.x) {
        atomicAdd(&histogram[bin_index], shared_histogram[bin_index]);
    }
}

template <class T> __global__ void scale_demodata_kernel(int buffer_size, T *buffer, float *scale_factor_buffer, T max_value) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = gridDim.x * blockDim.x;
    for (int element_index = index; element_index < buffer_size; element_index += stride) {
...
template <class T> void generate_demo_data(int buffer_size, T *buffer, curandGenerator_t rand_gen) {
...
template <class B_T, class H_T> void histogram(int threads, int blocks, int buffer_size, B_T *buffer, int bins, H_T *histogram, std::string kernel) {
    if (kernel == "basic") {
        histogram_kernel<<<blocks, threads>>>(buffer_size, buffer, bins, histogram);
    } else if (kernel == "shared") {
        histogram_kernel_shared<<<blocks, threads, sizeof(H_T) * bins>>>(buffer_size, buffer, bins, histogram);
    } else {
        std::cout << "Invalid kernel " << kernel << std::endl;
    }
}
...
int main(int argc, char **argv) {
    ...
    for (auto benchmark_config : benchmark_configs) {
        ...
        std::string kernel = benchmark_config.kernel;
        ...
        for (int run_index = 0; run_index < benchmark_config.runs; run_index++) {
            if (benchmark_config.bin_bytes == 4) {
                if (benchmark_config.element_bytes == 4) {
                    ...
                    histogram(threads, blocks, elements, element_buffer, bins, element_buffer, kernel);
