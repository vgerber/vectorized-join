...
__device__ void probe_kernel(db_table r_table, db_table s_table, db_hash_table s_hash_table, index_s_t *probe_sizes_buffer, filter_mask *probe_results, int hash_offset, int hash_full_size, int key_offset, int slots, hash_t *table_hashes, index_s_t *table_links, index_s_t *table_slots)
{
    ...
    for (int s_index = index; s_index < s_table.size; s_index += stride)
    {
        ...
        while (table_index)
        {
            ...
            int probe_index = s_index * hash_full_size + hash_offset + table_index;
...
__device__ void partial_probe_kernel(db_table r_table, db_table s_table, db_hash_table s_hash_table, index_s_t *probe_sizes_buffer, filter_mask *probe_results, int key_offset, int slots, hash_t *table_hashes, index_s_t *table_links, index_s_t *table_slots)
{
    ...
    for (int s_index = index; s_index < s_table.size; s_index += stride)
    {
        ...
        while (table_index)
        {
            ...
            int probe_index = s_index * r_table.size + table_index;
...
__global__ void build_and_partial_probe_kernel(db_table r_table, db_hash_table r_hash_table, db_table s_table, db_hash_table s_hash_table, index_s_t *probe_sizes_buffer, bool *probe_results, int key_offset, int slots)
{
    // hash table
    extern __shared__ hash_t* table[];
    hash_t *table_hashes = (hash_t*)&table[0];
    ...
    // probing
    // probe values from table s on hash table with values from r
    partial_probe_kernel(r_table, s_table, s_hash_table, probe_sizes_buffer, probe_results, key_offset, slots, table_hashes, table_links, table_slots);
}
...
__host__ void extract_probe_results(db_table r_table, db_table s_table, filter_mask *probe_buffer, index_s_t *probe_offsets_buffer, index_s_t *probe_sizes_buffer, db_table rs_table)
{
    int x_index = 0;
    int x_stride = 1;
    for (index_s_t s_buffer_index = x_index; s_buffer_index < s_table.size; s_buffer_index += x_stride)
        ...
        if (probe_size)
        {
            ...
            for (; probe_index < r_table.size; probe_index += probe_stride)
            {
                if (probe_buffer[probe_offset + probe_index])
                {
                    ...
                    index_s_t pair_offset = probe_offsets_buffer[s_buffer_index]++;
                    //printf("%d/%lld\n", pair_offset, rs_table.size);
                    //printf("%d %lld %lld\n", pair_offset, r_table.column_values[probe_index * r_table.column_count], s_table.column_values[s_buffer_index * s_table.column_count]);
                    //printf("%p %lld %p %lld\n", &rs_table.column_values[pair_offset * rs_table.column_count], rs_table.column_values[pair_offset * rs_table.column_count], &rs_table.column_values[pair_offset * rs_table.column_count +1], rs_table.column_values[pair_offset * rs_table.column_count + 1]);
                    rs_table.column_values[pair_offset * rs_table.column_count] = r_table.column_values[probe_index * r_table.column_count];
                    rs_table.column_values[pair_offset * rs_table.column_count + 1] = s_table.column_values[s_buffer_index * s_table.column_count];
...
void build_and_probe_gpu(db_table d_r_table, db_hash_table d_r_hash_table, db_table d_s_table, db_hash_table d_s_hash_table, db_table &d_joined_rs_table, int key_offset, cudaStream_t stream, ProbeConfig config)
{
    ...
    index_s_t *d_probe_sizes_buffer = nullptr;
    filter_mask *d_probe_results = nullptr;
    gpuErrchk(cudaMallocAsync(&d_probe_results, d_r_hash_table.size * d_s_hash_table.size * sizeof(filter_mask), stream));
    gpuErrchk(cudaMallocAsync(&d_probe_sizes_buffer, d_s_hash_table.size * sizeof(index_s_t), stream));
    gpuErrchk(cudaMemsetAsync(d_probe_sizes_buffer, 0, d_s_hash_table.size * sizeof(index_s_t), stream));
    ...
    build_and_partial_probe_kernel<<<blocks, config.build_threads, shared_mem, stream>>>(d_r_table, d_r_hash_table, d_s_table, d_s_hash_table, d_probe_sizes_buffer, d_probe_results, key_offset, slots);
#endif
    ...
    int sum_blocks = ceil(d_s_table.size / (float)(config.sum_n_per_thread * config.sum_threads));
    prefix_sum_kernel<<<sum_blocks, config.sum_threads, 0, stream>>>(d_s_hash_table.size, d_probe_sizes_buffer, d_probe_offsets_buffer);
    ...
    //std::cout << d_joined_rs_table.size << std::endl;
    cudaMallocAsync(&d_joined_rs_table.column_values, d_joined_rs_table.column_count * d_joined_rs_table.size * sizeof(column_t), stream);
    cudaMemsetAsync(d_joined_rs_table.column_values, 1, d_joined_rs_table.column_count * d_joined_rs_table.size * sizeof(column_t), stream);
    ...
    cudaFreeAsync(d_probe_sizes_buffer, stream);
